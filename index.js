...
function createURLWithParams(params, url) {
  ...
  for (let param in params) {
    urlObj.searchParams.append(param, urlObj[param]);
  }
...
function sortUrlParams(url) {
  ...
  let urlEntries = Array.from(urlObj.searchParams.entries());

  urlEntries.sort((a, b) => a[0].localeCompare(b[0]));

  let sortedSearchParams = new URLSearchParams(urlEntries);
